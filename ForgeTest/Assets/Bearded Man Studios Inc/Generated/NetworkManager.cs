...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetworkCubeNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetworkCubeNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkCubeNetworkObject.Length > 0 && NetworkCubeNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkCubeNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetworkCube instead, its shorter and easier to type out ;)")]
		public NetworkCubeBehavior InstantiateNetworkCubeNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkCubeNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkCubeBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkCubeBehavior>().networkObject = (NetworkCubeNetworkObject)obj;
		...
		public NetworkCubeBehavior InstantiateNetworkCube(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkCubeNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkCubeBehavior>();
